...
from .pricing import get_pricing, resolve_model, DEFAULT_MODEL
...
class TokenTracker:
    ...
    def per_model_summary(self):
        models: dict[str, list[TokenUsage]] = {}
        for r in self.records:
            models.setdefault(r.model, []).append(r)

        for model_name, recs in models.items():
            self._print_model_block(model_name, recs)

    def print_summary(self):
        self.per_model_summary() if len(
            set(r.model for r in self.records)
        ) > 1 else self._print_model_block(self.model, self.records)

    def _print_model_block(self, model_name: str, records: list[TokenUsage]):
        t = {
