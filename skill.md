...
| Multi-step agent orchestration | `high` (never use `max` unless critical) |
...
- After ~8-12 turns, summarize the conversation and start a fresh context window
...
### `DeepSeekMiddleware` — Zero-effort tracking
```python
...
### `strip_reasoning_smart()` — Per-message reasoning cleanup
```python
cleaned = MessageOptimizer.strip_reasoning_smart(messages)
# Automatically detects tool-call boundaries per message
```

### `DeepSeekTokenizer` — Accurate counting (with optional official tokenizer)
```python
...
3. **Compress system prompts** — strip verbose explanations (semantic-aware in v1.1)
4. **Summarize when approaching context limits** — detect via `should_summarize()` TokenBudget
