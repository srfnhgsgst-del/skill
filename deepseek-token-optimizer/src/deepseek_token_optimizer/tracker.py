...
class TokenUsage:
    ...
    def reasoning_ratio(self) -> float:
        if self.completion_tokens == 0: return 0.0
...
class TokenTracker:
    ...
    def track_request(
        ...
        reasoning_tokens: int = 0, total_tokens: Optional[int] = None,
        model: Optional[str] = None, tag: str = "",
    ):
        if total_tokens is None:
            total_tokens = prompt_tokens + completion_tokens
        self.records.append(TokenUsage(
            prompt_tokens=prompt_tokens, completion_tokens=completion_tokens,
            total_tokens=total_tokens, cache_hit_tokens=cache_hit,
            cache_miss_tokens=cache_miss, reasoning_tokens=reasoning_tokens,
            model=resolve_model(model) if model else self.model,
            tag=tag,
        ))

    def track_stream(self, prompt_tokens: int, stream_chunks: list, model: Optional[str] = None, tag: str = ""):
        ...
        for chunk in stream_chunks:
            ...
            if delta:
                ...
                if details: reasoning_tokens += getattr(details, "reasoning_tokens", 0) or 0
        self.track_request(prompt_tokens=prompt_tokens, completion_tokens=completion_tokens, reasoning_tokens=reasoning_tokens, model=model, tag=tag)

    def track_batch(self, usage_list: list[dict]):
        for usage in usage_list: self.parse_and_track(usage)

    def parse_and_track(self, api_response_usage: dict, model: Optional[str] = None, tag: str = ""):
        self.track_request(
            ...
            reasoning_tokens=api_response_usage.get("completion_tokens_details", {}).get("reasoning_tokens", 0),
            model=model, tag=tag,
        )
    ...
    @staticmethod
    def estimate_cost(messages: list, model: str = DEFAULT_MODEL, output_tokens_guess: int = 500, cache_miss_ratio: float = 1.0) -> dict:
    ...
    def total_cost(self, tag: str = "") -> float:
        cost = 0.0
        for r in self.records:
            if tag and r.tag != tag: continue
    ...
    def cost_by_tag(self) -> dict[str, float]:
        ...
        for r in self.records:
            ...
            tags[r.tag] = tags.get(r.tag, 0.0) + c
        return tags
    ...
    def total_tokens(self, tag: str = "") -> dict:
        recs = [r for r in self.records if not tag or r.tag == tag]
        return {
            ...
            "cache_hit_tokens": sum(r.cache_hit_tokens for r in recs),
            "cache_miss_tokens": sum(r.cache_miss_tokens for r in recs),
            "reasoning_tokens": sum(r.reasoning_tokens for r in recs),
        }

    def overall_cache_hit_rate(self, tag: str = "") -> float:
        totals = self.total_tokens(tag=tag)
        cache_total = totals["cache_hit_tokens"] + totals["cache_miss_tokens"]
        if cache_total == 0: return 0.0
    ...
    def export_html(self, filepath: str):
        ...
        tags = self.cost_by_tag()
        models = self.cost_by_model()
        tag_rows = "".join(f"<tr><td>{k}</td><td>${v:.4f}</td></tr>" for k, v in sorted(tags.items()))
        model_rows = "".join(f"<tr><td>{k}</td><td>${v:.4f}</td></tr>" for k, v in sorted(models.items()))
        html = f"""<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>DeepSeek Token Report</title>
<style>body{{font-family:sans-serif;max-width:800px;margin:40px auto;padding:0 20px}}
h1{{color:#1a1a2e}}table{{border-collapse:collapse;width:100%;margin:20px 0}}
th,td{{text-align:left;padding:10px;border-bottom:1px solid #ddd}}
th{{background:#1a1a2e;color:#fff}}tr:hover{{background:#f5f5f5}}
.metric{{font-size:1.5em;font-weight:bold;color:#4a90d9}}
.tag{{display:inline-block;padding:4px 8px;border-radius:4px;background:#e8f0fe;margin:2px}}</style></head><body>
<h1>DeepSeek Token Usage Report</h1>
<p>Generated: {time.strftime('%Y-%m-%d %H:%M')} | {len(self.records)} requests</p>
<table><tr><th>Metric</th><th>Value</th></tr>
<tr><td>Total Tokens</td><td class="metric">{totals['total_tokens']:,}</td></tr>
<tr><td>Prompt Tokens</td><td>{totals['prompt_tokens']:,}</td></tr>
<tr><td>Completion Tokens</td><td>{totals['completion_tokens']:,}</td></tr>
<tr><td>Reasoning Tokens</td><td>{totals['reasoning_tokens']:,}</td></tr>
<tr><td>Cache Hit Rate</td><td>{cache_rate:.1f}%</td></tr>
<tr><td>Total Cost</td><td class="metric">${cost:.6f}</td></tr></table>
"""
        if tags:
            html += f"<h2>Cost by Tag</h2><table><tr><th>Tag</th><th>Cost</th></tr>{tag_rows}</table>"
        if models:
            html += f"<h2>Cost by Model</h2><table><tr><th>Model</th><th>Cost</th></tr>{model_rows}</table>"
        html += "<h2>Request History</h2><table><tr><th>#</th><th>Model</th><th>Prompt</th><th>Completion</th><th>Cost</th><th>Tag</th></tr>"
        for i, r in enumerate(self.records, 1):
            p = get_pricing(r.model)
            c = (r.cache_hit_tokens/1_000_000)*p["cache_hit"]+(r.cache_miss_tokens/1_000_000)*p["cache_miss"]+(r.completion_tokens/1_000_000)*p["output"]
            t = f'<span class="tag">{r.tag}</span>' if r.tag else ""
            html += f"<tr><td>{i}</td><td>{r.model}</td><td>{r.prompt_tokens:,}</td><td>{r.completion_tokens:,}</td><td>${c:.6f}</td><td>{t}</td></tr>"
        html += "</table></body></html>"
        with open(filepath, "w", encoding="utf-8") as f: f.write(html)

    def print_summary(self):
        models: dict[str, list[TokenUsage]] = {}
        for r in self.records: models.setdefault(r.model, []).append(r)
        if len(models) > 1:
            for m, recs in models.items(): self._print_block(m, recs)
        else: self._print_block(self.model, self.records)
    ...
    def _print_block(self, model_name: str, records: list[TokenUsage]):
        ...
        p = get_pricing(model_name); cost = 0.0
        for r in records:
            cost += (r.cache_hit_tokens/1_000_000)*p["cache_hit"]+(r.cache_miss_tokens/1_000_000)*p["cache_miss"]+(r.completion_tokens/1_000_000)*p["output"]
        cache_total = t["cache_hit_tokens"] + t["cache_miss_tokens"]
        cache_rate = (t["cache_hit_tokens"]/cache_total*100) if cache_total > 0 else 0
        print("="*60); print(f"  DeepSeek Token Usage Summary ({model_name})"); print("="*60)
